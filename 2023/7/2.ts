...
export default class Solution implements Answer {
  ...
  getHandRank(hand: [string, number]): number {
    ...
    // if there's J's just put any j's we have
    // to the highest amount of cards. will always
    // be the best option
    if (cardCount.has('J')) {
      const jCount = cardCount.get('J')
      cardCount.delete('J')
      let max = -Infinity
      let picked = ''
      for (const [char, count] of cardCount) {
        if (count > max) {
          max = count
          picked = char
        }
      }
      cardCount.set(picked, cardCount.get(picked) + jCount)
    }
